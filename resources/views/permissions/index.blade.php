                        ...
                        <table class="min-w-full table-auto">
                            <thead>
                                <tr class="bg-gray-100 dark:bg-gray-700">
                                    ...
                                    <th class="px-4 py-2 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Permission Name') }}</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                @foreach ($permissions as $permission)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $permission->name }}</td>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-center space-x-2">
                                                ...
                                                @can('delete permissions')
                                                    <form method="POST" action="{{ route('permissions.destroy', $permission) }}" class="inline" x-data="{ confirm: false }" @submit.prevent="if (!confirm) { confirm = true; return false; }">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                                                        ...
                                                        <div x-show="confirm" x-cloak
                                                            ...
                                                            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-sm w-full">
                                                                <p class="text-gray-900 dark:text-gray-100 mb-4">{{ __('Delete this permission?') }}</p>
